...
public class Lotto2 extends JFrame implements ActionListener{
	...
	public Lotto2(){
		...
		try{
			...
			SwingUtilities.updateComponentTreeUI(Lotto2.this);
		}catch(Exception e){ }
		writeTest();
		start_display();
	}

	public Lotto2(String str)
	{
	...
	public void start_display(){
		...
		pane.add(new JLabel("바꿀 자리:"));

		gbc.gridy=1;
		//pane.add(new JLabel("변경 번호:"), gbc);
		pane.add(new JLabel("변경 번호:"));

		gbc.gridx=1;
		gbc.gridy=0;
		gbc.anchor = GridBagConstraints.EAST;
		//pane.add(originalField, gbc);
		pane.add(originalField);

		gbc.gridy=1;
		//pane.add(changeField, gbc);
		pane.add(changeField);
